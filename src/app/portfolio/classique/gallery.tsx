...
export function ClassiqueGallery() {
  ...
  const prev = useCallback(() => {
    setOpenIndex((i) => (i === null ? null : i === 0 ? classiqueImages.length - 1 : i - 1));
  }, []);
  const next = useCallback(() => {
    setOpenIndex((i) => (i === null ? null : i === classiqueImages.length - 1 ? 0 : i + 1));
  }, []);
      ...
      {openIndex !== null && (
        ...
        >
          ...
          <p className="absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full border border-white/10 bg-dark/60 px-4 py-1.5 text-sm text-white/80">
            {openIndex + 1} / {classiqueImages.length}
          </p>
        </div>
      )}
    </>
